...
**`extension/content/content.js` の検出ロジックを変更したら必ず実行する。**

依存: `jsdom`（`npm install --no-save jsdom` で導入済み）。`package.json` は意図的に置いていない。

## リリース管理

この拡張は単独リポジトリとして扱う。Chrome / Firefox のリリースは `manifests/<target>.json` で分離し、`scripts/package-release.ps1` で target 別パッケージを作る。
...
`detectSale()` の実装は **`extension/shared/detect.js` のみ**。

- `extension/shared/detect.js` — 唯一の実装。UMD 形式でブラウザ/Node.js 両対応
- `extension/content/content.js` — manifest で先行読み込みされた `window.__WSP__` から取得
- `verify-detect.mjs` — `createRequire('./extension/shared/detect.js')` で直接 import

**`extension/shared/detect.js` を変更したら `node verify-detect.mjs` で回帰検証すること。**
...
`manifests/chrome.json` / `manifests/firefox.json` の `matches`:
...
- 結果は `extension/results/results.html` にリスト別集約表示
